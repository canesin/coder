...
const NO_SECRETS = { blockSecrets: false };
...
test("ppcommit: skip via config", async () => {
  ...
  const r = await runPpcommitNative(repo, { skip: true, blockSecrets: false });
...
test("ppcommit: detects TODO comment", async () => {
  ...
  const r = await runPpcommitNative(repo, NO_SECRETS);
...
test("ppcommit branch: no files changed since base", async () => {
  ...
  const r = await runPpcommitBranch(repo, "main", NO_SECRETS);
  assert.equal(r.exitCode, 0);
...
test("ppcommit all: checks all files in the repo", async () => {
  ...
  run("git", ["add", "a.js"], repo);
  run("git", ["commit", "-m", "add a.js"], repo);
  const r = await runPpcommitAll(repo, NO_SECRETS);
  assert.equal(r.exitCode, 1);
  assert.match(r.stdout, /TODO/);
  assert.match(r.stdout, /a\.js:1/);
});
...
test("ppcommit: gitleaks missing from PATH produces actionable error", async () => {
  ...
  try {
    ...
    const { runPpcommitNative } = await import(
      `file://${srcPath}?cacheBust=${Date.now()}`
    );
    await runPpcommitNative(repo, { blockSecrets: true });
    assert.fail("should have thrown an error");
  } catch (e) {
    const out = String(e?.message || e);
